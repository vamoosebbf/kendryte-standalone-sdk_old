...
int irq_gpiohs2(void *ctx)
{
    ...
    if(!irq_flag)
    {
        ...
        gpiohs_set_pin(GPIO_WLED, GPIO_PV_HIGH);
    } else
    {
        gpiohs_set_pin(GPIO_LED, GPIO_PV_LOW);
        gpiohs_set_pin(GPIO_WLED, GPIO_PV_LOW);
    }
    return 0;
}
...
int main(void)
{
    ...
    fpioa_set_function(PIN_LEDW, FUNC_GPIOHS4);
    gpiohs_set_drive_mode(GPIO_LED, GPIO_DM_OUTPUT);
    gpiohs_set_drive_mode(GPIO_WLED, GPIO_DM_OUTPUT);
    gpio_pin_value_t value = GPIO_PV_HIGH;
    gpiohs_set_pin(GPIO_LED, !value);
    gpiohs_set_pin(GPIO_WLED, value);
